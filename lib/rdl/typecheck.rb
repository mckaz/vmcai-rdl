module RDL::Typecheck
  ...
  class Env
    ...
    def mark_here(var)
      result = Env.new
      result.env = @env.merge(var => {type: @env[var][:type], fixed: @env[var][:fixed], here: true})
      return result
    end
    
    def def_here?(var)
      return @env[var] && @env[var][:here]
    end

    def unmark_here
      @env.each_key { |var| @env[var][:here] = false }
      return self
    end
  ...
  def self.typecheck(klass, meth, ast=nil, ast_types=nil)
    ...
    types.each { |type|
      ...
      if meth == :initialize
        ...
        tmp_typ = (if (type.ret.is_a?(RDL::Type::AnnotatedArgType) || type.ret.is_a?(RDL::Type::DependentArgType)) then type.ret.type else type.ret end)
        error :bad_initialize_type, [], ast unless tmp_typ.is_a?(RDL::Type::VarType) && tmp_typ.name == :self
      end
      inst = {self: self_type}
      type = type.instantiate inst
      _, targs = args_hash({}, Env.new, type, args, ast, 'method', ast_types)
      ...
      begin
        ...
        if body.nil?
          ...
          ast_types[body.object_id] = body_type if ast_types
        else
          _, body_type = tc(scope, Env.new(targs.merge(scope[:captured])), body, ast_types)
        end
      end until old_captured == scope[:captured]
      ret_type = if (type.ret.is_a? RDL::Type::AnnotatedArgType) || (type.ret.is_a? RDL::Type::DependentArgType) then type.ret.type else type.ret end
      body_type_compare = if (body_type.is_a? RDL::Type::AnnotatedArgType) || (body_type.is_a? RDL::Type::DependentArgType) then body_type.type else body_type end
      error :bad_return_type, [body_type.to_s, type.ret.to_s], body unless body.nil? || meth == :initialize || body_type_compare <= ret_type#type.ret
    }
  ...
  def self.args_hash(scope, env, type, args, ast, kind, ast_types=nil)
    targs = Hash.new
    ...
    args.children.each { |arg|
      ...
      if arg.type == :arg
        ...
        error :type_arg_kind_mismatch, [kind, 'vararg', 'required'], arg if (targ.vararg? && !(kind == 'block')) ## TODO: correctly handle blocks
        targs[arg.children[0]] = targ
        ast_types[arg.children[0].object_id] = targ if ast_types
        tpos += 1
        ...
        ast_types[arg.children[0].object_id] = targ.type if ast_types
        tpos += 1
        ...
        ret_type = RDL::Type::GenericType.new(RDL.types[:array], targ.type)
        targs[arg.children[0]] = ret_type
        ast_types[arg.children[0].object_id] = ret_type if ast_types
        tpos += 1
        ...
        kw_args_matched << kw
        targs[kw] = tkw
        ast_types[kw.object_id] = tkw if ast_types
        ...
        kw_args_matched << kw
        targs[kw] = tkw.type
        ast_types[kw.object_id] = tkw.type if ast_types
        ...
        ret_type = RDL::Type::GenericType.new(RDL.types[:hash], RDL.types[:symbol], targ.rest)
        targs[arg.children[0]] = ret_type
        ast_types[arg.children[0].object_id] = ret_type if ast_types
        ...
        ast_types[arg.children[0].object_id] = type.block if ast_types
    ...
    if (tpos == type.args.length - 1) && type.args[tpos].is_a?(RDL::Type::FiniteHashType)
      ...
      error :type_args_more, [kind, kind], args if type.args.length != tpos
    end
    return [env, targs]
  end
  ...
  # [+ ast_types +] if provided is a hash mapping ast nodes (by object_id) to types. only needed for verification.
  ...
  def self.tc(scope, env, e, ast_types=nil)
    case e.type
    when :nil
      ast_types[e.object_id] = RDL.types[:nil] if ast_types
      [env, RDL.types[:nil]]
    when :true
      ast_types[e.object_id] = RDL.types[:true] if ast_types
      [env, RDL.types[:true]]
    when :false
      ast_types[e.object_id] = RDL.types[:false] if ast_types
      [env, RDL.types[:false]]
    when :complex, :rational, :str, :string # constants
      ast_types[e.object_id] = RDL::Type::NominalType.new(e.children[0].class) if ast_types
      [env, RDL::Type::NominalType.new(e.children[0].class)]
    when :int, :float, :sym # singletons
      ast_types[e.object_id] = RDL::Type::SingletonType.new(e.children[0]) if ast_types
    ...
    when :dstr, :xstr # string (or execute-string) with interpolation
      ...
      ast_types[e.object_id] =  RDL.types[:string] if ast_types
    ...
    when :array
      ...
      e.children.each { |ei|
        if ei.type == :splat
          envi, ti = tc(scope, envi, ei.children[0], ast_types);
          ...
          envi, ti = tc(scope, envi, ei, ast_types);
          tis << ti
        end
      }
      if is_array
    ...
    when :hash
      ...
      e.children.each { |p|
        ...
        if p.type == :pair
          envi, tleft = tc(scope, envi, p.children[0], ast_types)
          tlefts << tleft
          envi, tright = tc(scope, envi, p.children[1], ast_types)
          trights << tright
          is_fh = false unless tleft.is_a?(RDL::Type::SingletonType)
        elsif p.type == :kwsplat
          envi, tkwsplat = tc(scope, envi, p.children[0], ast_types)
      ...
      if is_fh
        ...
        ret = RDL::Type::FiniteHashType.new(fh, nil)
        ast_types[e.object_id] = ret if ast_types
        [envi, ret]
      else
        tleft = RDL::Type::UnionType.new(*tlefts)
        tright = RDL::Type::UnionType.new(*trights)
        ret = RDL::Type::GenericType.new(RDL.types[:hash], tleft, tright)
        ast_types[e.object_id] = ret if ast_types
        [envi, ret]
      end
      #TODO test!
#    when :kwsplat # TODO!
    ...
    when :lvar, :ivar, :cvar, :gvar
      tc_var(scope, env, e.type, e.children[0], e, ast_types)
    when :lvasgn, :ivasgn, :cvasgn, :gvasgn
      ...
      envright, tright = tc(scope, env, e.children[1], ast_types)
      tc_vasgn(scope, envright, e.type, x, tright, e, ast_types)
    when :masgn
      ...
      envi, tright = tc(scope, env, e.children[1], ast_types)
      ...
      if tright.is_a? RDL::Type::TupleType
        ...
        if splat_ind
          if splat_ind > 0
            lhs[0..splat_ind-1].each { |left|
              ...
              envi, _ = tc_vasgn(scope, envi, left.type, left.children[0], rhs.shift, left, ast_types)
            }
          end
          lhs[splat_ind+1..-1].reverse_each { |left|
            # after splat
            error :masgn_bad_lhs, [], left if rhs.empty?
            envi, _ = tc_vasgn(scope, envi, left.type, left.children[0], rhs.pop, left, ast_types)
          }
          splat = lhs[splat_ind]
          envi, _ = tc_vasgn(scope, envi, splat.children[0].type, splat.children[0].children[0], RDL::Type::TupleType.new(*rhs), splat, ast_types)
          ast_types[e.object_id] = tright if ast_types          
          [envi, tright]
        else
          error :masgn_num, [rhs.length, lhs.length], e unless lhs.length == rhs.length
          lhs.zip(rhs).each { |left, right|
            envi, _ = tc_vasgn(scope, envi, left.type, left.children[0], right, left, ast_types)
          }
          ast_types[e.object_id] = tright if ast_types          
          [envi, tright]
        end
        ...
        lhs.each { |asgn|
          if asgn.type == :splat
            ...
            envi, _ = tc_vasgn(scope, envi, asgn.type, asgn.children[0], tasgn, asgn, ast_types)
          end
        }
        ast_types[e.object_id] = tright if ast_types
        [envi, tright]
    ...
    when :op_asgn
      if e.children[0].type == :send
        ...
        envleft, trecv = tc(scope, env, e.children[0].children[0], ast_types) # recv
        elargs = e.children[0].children[2]

        if elargs
          envleft, elargs = tc(scope, envleft, elargs, ast_types)
          largs = [elargs]
        else
          largs = []
        end

        tloperand = tc_send(scope, envleft, trecv, meth, largs, nil, e.children[0], ast_types) # call recv.meth()
        envoperand, troperand = tc(scope, envleft, e.children[2], ast_types) # operand
        tright = tc_send(scope, envoperand, tloperand, e.children[1], [troperand], nil, e, ast_types) # recv.meth().op(operand)

        tright = largs.push(tright) if largs
        mutation_meth = (meth.to_s + '=').to_sym
        tres = tc_send(scope, envoperand, trecv, mutation_meth, tright, nil, e, ast_types) # call recv.meth=(recvt.meth().op(operand))
        [envoperand, tres]
      else
        ...
        #ast_types[e.children[0].object_id] = env[x] if ast_types 
        envi, trecv = tc_var(scope, env, @@asgn_to_var[e.children[0].type], x, e.children[0], ast_types) # var being assigned to
        envright, tright = tc(scope, envi, e.children[2], ast_types) # operand
        trhs = tc_send(scope, envright, trecv, e.children[1], [tright], nil, e, ast_types)
        tc_vasgn(scope, envright, e.children[0].type, x, trhs, e, ast_types)
      end
    ...
    when :const
      ...
      when TrueClass, FalseClass, Complex, Rational, Fixnum, Bignum, Float, Symbol, Class
        ret = RDL::Type::SingletonType.new(c)
        ast_types[e.object_id] = ret if ast_types
        [env, ret]
      when Module
        t = RDL::Type::SingletonType.new(const_get(e.children[1]))
        ast_types[e.object_id] = t if ast_types
        [env, t]
      else
        ret = RDL::Type::NominalType.new(const_get(e.children[1]).class)
        ast_types[e.object_id] = ret if ast_types
        [env, ret]
      end
    when :defined?
    ...
    when :send, :csend
      ...
      return [env, RDL.types[:nil]] if e.children[1] == :exception? ## for verification
      return tc_var_type(scope, env, e) if e.children[1] == :var_type && e.children[0].nil?      
      return tc_type_cast(scope, env, e, ast_types) if e.children[1] == :type_cast && scope[:block].nil?
      ...
      scope_merge(scope, block: nil) { |sscope|
        e.children[2..-1].each { |ei|
          if ei.type == :splat
            envi, ti = tc(sscope, envi, ei.children[0], ast_types)
            if ti.is_a? RDL::Type::TupleType
            ...
            ti = if ti.optional? then ti.type else ti end 
            ti = tc_send(sscope, envi, ti, :to_proc, [], nil, ei) unless ti.is_a? RDL::Type::MethodType
            ### ast_types needed here??
            block = [ti, ei]
          else
            envi, ti = tc(sscope, envi, ei, ast_types)
            tactuals << ti
          end
        }
        envi, trecv = if e.children[0].nil? then [envi, envi[:self]] else tc(sscope, envi, e.children[0], ast_types) end # if no receiver, self is receiver
        ast_types[e.children[0].object_id] = trecv if (ast_types && !e.children[0].nil?)
        ret = tc_send(sscope, envi, trecv, e.children[1], tactuals, block, e, ast_types).canonical
        ast_types[e.object_id] = ret if ast_types
        [envi, ret]
      }
    when :yield
...
RUBY
    ...
    when :block
      ...
      scope_merge(scope, block: [e.children[1], e.children[2]]) { |bscope|
        tc(bscope, env, e.children[0], ast_types)
      }
    when :and, :or
      envleft, tleft = tc(scope, env, e.children[0], ast_types)
      envright, tright = tc(scope, envleft, e.children[1], ast_types)
      if tleft.is_a? RDL::Type::SingletonType
        if e.type == :and
          if tleft.val then
            ...
            ast_types[e.object_id] = tleft if ast_types
            [envleft, tleft]
          end
        else # e.type == :or
          if tleft.val then
    ...
    #   end
    when :if
      envi, tguard = tc(scope, env, e.children[0], ast_types) # guard; any type allowed
      ...
      if e.children[1].nil? #then
        envleft, tleft = [envi, RDL.types[:nil]]
        ast_types[e.children[1].object_id] = RDL.types[:nil] if ast_types
      else
        envleft, tleft = tc(scope, envi, e.children[1], ast_types)
      end
      #envright, tright = if e.children[2].nil? then [envi, $__rdl_nil_type] else tc(scope, envi, e.children[2]) end # else
      if e.children[2].nil?
        envright, tright = [envi, RDL.types[:nil]]
        ast_types[e.children[2].object_id] = RDL.types[:nil] if ast_types
      else
        envright, tright = tc(scope, envi, e.children[2], ast_types)
      end
      if tguard.is_a? RDL::Type::SingletonType
        ...
        unitype = RDL::Type::UnionType.new(tleft, tright).canonical
        ast_types[e.object_id] = unitype if ast_types
        [Env.join(e, envleft, envright), unitype]
      end
    when :case
      envi = env
    ...
    when :return
      ...
      if e.children[0]
         env1, t1 = tc(scope, env, e.children[0], ast_types)
      else
    ...
    when :begin, :kwbegin # sequencing
      envi = env
      ti = nil
      e.children.each { |ei| envi, ti = tc(scope, envi, ei, ast_types) }
      ast_types[e.object_id] = ti if ast_types
      [envi, ti]
    when :ensure
  ...
  def self.tc_var(scope, env, kind, name, e, ast_types=nil)
    case kind
    when :lvar  # local variable
      #error :undefined_local_or_method, [name], e unless env.has_key? name
      error :outer_env_not_fixed, [name], e if (scope[:outer_env] && (scope[:outer_env].has_key? name) && (not (scope[:outer_env].fixed? name)) && (not (env.def_here?(name))))
      #capture(scope, name, env[name].canonical) if scope[:outer_env] && (scope[:outer_env].has_key? name) && (not (scope[:outer_env].fixed? name))
      if !(env.has_key?(name))
        error :undefined_local_or_method, [name], e unless @lvarmap.has_key?(e.object_id)
        ast_types[e.object_id] = @lvarmap[e.object_id]
        [env, @lvarmap[e.object_id]]
      elsif scope[:captured] && scope[:captured].has_key?(name) then
        @lvarmap[e.object_id] = scope[:captured][name] if @in_dyn
        ast_types[e.object_id] = scope[:captured][name] if ast_types
        [env, scope[:captured][name]]
      else
        @lvarmap[e.object_id] = env[name].canonical if @in_dyn
        ast_types[e.object_id] = env[name].canonical if ast_types
    ...
    when :ivar, :cvar, :gvar
      ...
      var_type = RDL.info.get(klass, name, :type).canonical
      ast_types[e.object_id] = var_type if ast_types
      [env, var_type]
    else
  ...
  def self.tc_vasgn(scope, env, kind, name, tright, e, ast_types = nil)
    if (tright.is_a?(RDL::Type::AnnotatedArgType) || tright.is_a?(RDL::Type::DependentArgType)) then tright = tright.type end
    case kind
    when :lvasgn
      if ((scope[:captured] && scope[:captured].has_key?(name)) ||
        ...
        t = scope[:captured][name]
        ast_types[e.object_id] = t if ast_types
        [env.mark_here(name), t]
      elsif (env.fixed? name)
        error :vasgn_incompat, [tright, env[name]], e unless tright <= env[name]
        t = tright.canonical
        ast_types[e.object_id] = t if ast_types
        @lvarmap[e.object_id] = t # save type to check during verification
        env = env.mark_here(name)
        [env.mark_here(name), t]
      else
        t = tright.canonical
        ast_types[e.object_id] = t if ast_types
        env = env.bind(name, tright)
        [env.bind(name, tright).mark_here(name), t]
      end
    ...
    when :send
      ...
      t = tc_send(scope, envi, trecv, meth, [*typs, tright], nil, e)
      ast_types[e.object_id] = t if ast_types
      [envi, t] # call receiver.meth(other args, tright)
    else
      raise RuntimeError, "unknown kind #{kind}"
    end
  end
  ...
  def self.tc_type_cast(scope, env, e, ast_types)
    ...
    tc(scope, env, e.children[0], ast_types) if ast_types # still want types for receiver if using ast_types
    ast_types[e.children[0].object_id] = typ if ast_types  # receiver
    ast_types[e.object_id] = ast_types[e.object_id] = typ if ast_types
    [env1, typ]
  end
  ...
  def self.tc_send(scope, env, trecvs, meth, tactuals, block, e, ast_types=nil)
    scope[:exn] = Env.join(e, scope[:exn], env) if scope.has_key? :exn # assume this call might raise an exception

    # convert trecvs to array containing all receiver types
    trecvs = trecvs.canonical
    trecvs = (trecvs.is_a?(RDL::Type::AnnotatedArgType) || trecvs.is_a?(RDL::Type::DependentArgType)) ? trecvs.type : trecvs
    trecvs = if trecvs.is_a? RDL::Type::UnionType then trecvs.types else [trecvs] end

    trets = []
    trecvs.each { |trecv|
      trets.concat(tc_send_one_recv(scope, env, trecv, meth, tactuals, block, e, ast_types))
    }
    trets.map! {|t| (t.is_a?(RDL::Type::AnnotatedArgType) || t.is_a?(RDL::Type::DependentArgType)) ? t.type : t}
    ret_type = RDL::Type::UnionType.new(*trets)
    ast_types[e.object_id] = ret_type if ast_types
    return ret_type
  end
  ...
  def self.tc_send_one_recv(scope, env, trecv, meth, tactuals, block, e, ast_types=nil)
    return tc_send_class(trecv, e) if (meth == :class) && (tactuals.empty?)
    tmeth_inter = [] # Array<MethodType>, i.e., an intersection types
    trecv = trecv.type if (trecv.is_a?(RDL::Type::AnnotatedArgType) || trecv.is_a?(RDL::Type::DependentArgType))
    case trecv
    when RDL::Type::SingletonType
      if trecv.val.is_a? Class or trecv.val.is_a? Module
        ...
        #inst = if meth == :new then {self: RDL::Type::NominalType.new(trecv.val)} else {self: trecv} end
        inst = {self: trecv}
    ...
    when RDL::Type::MethodType
      if meth == :call
        ...
        # treat as Proc
        tc_send_one_recv(scope, env, RDL.types[:proc], meth, tactuals, block, e, ast_types)
      end
    when RDL::Type::OptionalType
      if meth == :call
        # Special case - invokes the Proc
        tmeth_inter = [trecv.type]
      else
        # treat as Proc
        tc_send_one_recv(scope, env, RDL.types[:proc].type, meth, tactuals, block, e, ast_types)
      end
    else
    ...
    RDL::Type.expand_product(tactuals).each { |tactuals_expanded|
      ...
      tmeth_inter.each { |tmeth| # MethodType
        if ((tmeth.block && block) || (tmeth.block.nil? && block.nil?) || (tmeth.block && tmeth.block.optional?))
          tmeth_inst = tc_arg_types(tmeth, tactuals_expanded)
          if tmeth_inst
            if block
              pre_in_dyn = @in_dyn
              @in_dyn = true if (meth == :define_method) || (meth == :define_singleton_method) || (meth == :rdl_define_method)
              if (meth == :rdl_define_method)
                str_typ = e.children[3].children[0] ## should be string type passed into rdl_define_method
                blk_typ = RDL.parser.scan_str str_typ
              else
                blk_typ = tmeth.block
              end
                begin 
                  tc_block(scope, env, blk_typ, block, tmeth_inst, ast_types)
                rescue => err
                  block_mismatch = true
                end
                @in_dyn = pre_in_dyn
            ...
            if trecv.is_a?(RDL::Type::SingletonType) && meth == :new
              ...
              trets_tmp << tmeth.ret.instantiate(tmeth_inst) unless block_mismatch# found a match for this subunion; add its return type to trets_tmp
...
RUBY
      ...
      name = if trecv.is_a?(RDL::Type::SingletonType) && trecv.val.is_a?(Class) && (meth == :new) then
        ...
        trecv.to_s
      elsif (trecv.is_a?(RDL::Type::MethodType) || (trecv.is_a?(RDL::Type::OptionalType) && trecv.type.is_a?(RDL::Type::MethodType)))
        'Proc'
      else
  ...
  def self.tc_send_class(trecv, e)
    trecv = trecv.type if (trecv.is_a?(RDL::Type::DependentArgType) || trecv.is_a?(RDL::Type::AnnotatedArgType))
    case trecv
    when RDL::Type::SingletonType
    ...
    when RDL::Type::NominalType
      return [RDL::Type::SingletonType.new(trecv.klass)]
    when RDL::Type::GenericType
      return [RDL::Type::SingletonType.new(trecv.base.klass)]
    when RDL::Type::TupleType
      return [RDL::Type::SingletonType.new(Array)]
    when RDL::Type::FiniteHashType
      return [RDL::Type::SingletonType.new(Hash)]
    when RDL::Type::VarType
      error :recv_var_type, [trecv], e
    when RDL::Type::MethodType
      return [RDL::Type::SingletonType.new(Proc)]
    else
  ...
  def self.tc_arg_types(tmeth, tactuals)
    ...
    tformals = tmeth.args
    tactuals.map! {|t| (t.is_a?(RDL::Type::AnnotatedArgType) || t.is_a?(RDL::Type::DependentArgType)) ? t.type : t}
    until states.empty?
      ...
      t = tformals[formal]
      if t.instance_of?(RDL::Type::AnnotatedArgType) || t.instance_of?(RDL::Type::DependentArgType)
        t = t.type
      end
      case t
      when RDL::Type::OptionalType
  ...
  def self.tc_block(scope, env, tblock, block, inst, ast_types=nil)
    # TODO self is the same *except* instance_exec or instance_eval
    tblock = if tblock.optional? then tblock.type else tblock end
    ...
    if block[0].is_a? RDL::Type::MethodType
      ...
      scope_merge(scope, outer_env: env.unmark_here) { |bscope|
        # note: okay if outer_env shadows, since nested scope will include outer scope by next line
        env = env.merge(Env.new(targs))
        _, body_type = if body.nil? then [nil, RDL.types[:nil]] else tc(bscope, env.merge(Env.new(targs)), body, ast_types) end
        ast_types[body.object_id] = body_type if ast_types
  ...
  def self.lookup(scope, klass, name, e)
    ...
    if the_klass.instance_of?(Module)
      anc = Module.ancestors # for any module mod, calling mod.ancestors  will not return Module.ancestors, which mod inherits from
    else
      anc = []
    end
    is_singleton = RDL::Util.has_singleton_marker(the_klass.to_s)
    included = if the_klass.instance_of?(Module) then the_klass.included_modules + [PP::ObjectMixin, Kernel] else the_klass.included_modules end # mod.included will not return PP:ObjectMixin and Kernel for some reason
    (the_klass.ancestors + anc)[1..-1].each { |ancestor|
...
class Diagnostic < Parser::Diagnostic
  ...
  RDL_MESSAGES = {
    ...
    outer_env_not_fixed: "local variable `%s' must have a fixed type since it is defined in an outer scope. use `var_type` method to fix its type.",
