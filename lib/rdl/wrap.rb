...
#            puts "Intercepted #{full_method_name}(\#{args.join(", ")}) { \#{blk} }, inst = \#{inst.inspect}"
            ...
            RDL::Verify.verify_type(klass, meth) if RDL.info.get(klass, meth, :verify) == :call
          ...
          RDL.wrap_switch.off {
            ...
            if matches
              if meth.to_sym == :initialize
                types.each { |t|
                  ret_type = t.ret
                  ret_type = ret_type.type if (ret_type.is_a?(RDL::Type::AnnotatedArgType) || ret_type.is_a?(RDL::Type::DependentArgType))
                  raise ArgumentError, "Initialize method must be annotated with return type `self` or a generic type where base is `self`. #{full_method_name} has incorrect return type." unless ((ret_type.is_a?(RDL::Type::VarType) && ret_type.name == :self) || (ret_type.is_a?(RDL::Type::GenericType) && ret_type.base.is_a?(RDL::Type::VarType) && ret_type.base.name == :self)) 
                }
               else
...
#    meth = :initialize if meth && meth.to_sym == :new  # actually wrap constructor
  ...
  def self.process_assert_args(default_class, *args, &blk)
    klass = meth = nil
    default_class = "Object" if (default_class.is_a? Object) && (default_class.to_s == "main")
    if args.size == 2
      klass = class_to_string args[0]
      slf, meth = meth_to_sym args[1]
    elsif args.size == 1
    ...
    klass = RDL::Util.add_singleton_marker(klass) if slf #necessary???
    return [klass, meth]
  end
  ...
  def self.meth_to_sym(meth)
    ...
    meth = meth.to_s
    meth =~ /^(.*\.)?(.*)$/
    raise RuntimeError, "Only self.method allowed, got method name '#{meth.to_s}'" if $1 && $1 != "self."
    return [$1, $2.to_sym]
  end
  ...
  def self.do_method_added(the_self, sing, klass, meth)
    ...
    if RDL.deferred.size > 0
      ...
      a.each { |prev_klass, kind, contract, h|
        ...
        RDL.info.add(klass, meth, :modifies, h[:modifies]) if !h[:modifies].nil?
        RDL.info.add(klass, meth, :assumes, h[:assumes])
        ...
        unless !h.has_key?(:verify) || RDL.info.set(klass, meth, :verify, h[:verify])
          raise RuntimeError, "Inconsistent verify flag on #{RDL::Util.pp_klass_method(klass, meth)}"
        end
        RDL::Typecheck.typecheck(klass, meth) if h[:typecheck] == :now
        RDL::Verify.verify_assert(klass, meth) if kind == :assert
        ...
        if (h[:verify] && h[:verify] != :call)
          RDL.to_verify_type[h[:verify]] = Set.new unless RDL.to_verify_type[h[:verify]]
          RDL.to_verify_type[h[:verify]].add([klass, meth])
        end
      }
    end
    ...
    if RDL.to_verify_assert.member? [klass, meth]
      RDL.to_verify_assert.delete [klass, meth]
      RDL::Verify.verify_assert(klass, meth)
    end
    ...
    if RDL.to_verify_type[:now].member? [klass, meth]
      RDL.to_verify_type[:now].delete [klass, meth]
      RDL::Verify.verify_type(klass, meth)
    end
...
class Module
  # monkey patching include method so that any included module invokes a callback
  ...
  def included_callback(mod_name)
    RDL.included_in[mod_name] = [] unless RDL.included_in[mod_name]
    RDL.included_in[mod_name] << self
    RDL.includes[self] = [] unless RDL.includes[self]
    RDL.includes[self] << mod_name
    if RDL.mixin_types[mod_name]
      RDL.mixin_types[mod_name].each { |meth, typ, wrap, typecheck, verify, modifies, pure|
        case self
        when Class
          type(self, meth, typ, wrap: wrap, typecheck: typecheck, verify: verify, modifies: modifies, pure: pure) #TODO: add to subclasses?
        when Module
          add_mixin_type(self, meth, typ, wrap: wrap, typecheck: typecheck, verify: verify, modifies: modifies, pure: pure) #TODO: is this right?
        end
      }
    end
  end
...
class Object
  ...
  # [+ verify +] is similar to typecheck
  #
  ...
  def type(*args, wrap: RDL::Config.instance.type_defaults[:wrap], typecheck: RDL::Config.instance.type_defaults[:typecheck], verify: RDL::Config.instance.type_defaults[:verify], modular: RDL::Config.instance.type_defaults[:modular], modifies: RDL::Config.instance.type_defaults[:modifies], pure: RDL::Config.instance.type_defaults[:pure], assumes: RDL::Config.instance.type_defaults[:assumes])
    ...
    if RDL::Util.has_singleton_marker(klass)
      no_mark = RDL::Util.remove_singleton_marker(klass)
      sing = true
    else
      no_mark = klass
      sing = false
    end
    kl_no_mark = RDL::Util.to_class(no_mark)
    if kl_no_mark.instance_of?(Module) || kl_no_mark.instance_of?(Class)
      #raise RuntimeError, "Type annotations for Modules must come with an modular flag. Annotation for module #{kl_no_mark} #{if sing then 'singleton' else 'instance' end} method #{meth} is missing modular flag." if modular.nil?
      if modular
        raise RuntimeError, "Method name must be explicitly provided in type annotation when modular is true." unless meth
        RDL.info.add(klass, meth, :type, type)
        RDL.info.add(klass, meth, :modifies, modifies)
        meth = "self."+meth.to_s if sing
        add_mixin_type(kl_no_mark, meth, type, wrap: wrap, typecheck: typecheck, verify: verify, modifies: modifies, pure: pure)
        return nil
      end
    elsif kl_no_mark.is_a?(Class)
      #raise RuntimeError, "modular flag cannot be true for class types" if modular
    end
    if meth
...
#          warn "#{RDL::Util.pp_klass_method(klass, meth)}: methods that end in ? should have return type %bool"
      ...
      RDL.info.add(klass, meth, :type, type)
      RDL.info.add(klass, meth, :modifies, modifies)
      RDL.info.add(klass, meth, :assumes, assumes)
      unless RDL.info.set(klass, meth, :typecheck, typecheck)
        raise RuntimeError, "Inconsistent typecheck flag on #{RDL::Util.pp_klass_method(klass, meth)}"
      end
      unless RDL.info.set(klass, meth, :verify, verify)
        raise RuntimeError, "Inconsistent verify flag on #{RDL::Util.pp_klass_method(klass, meth)}"
      end
      if wrap || typecheck == :now || verify == :now
        if RDL::Util.method_defined?(klass, meth) || meth == :initialize
          ...
          RDL::Verify.verify_type(klass, meth) if verify == :now
          ...
          if (verify && verify != :call)
            RDL.to_verify_type[verify] = Set.new unless RDL.to_verify_type[verify]
            RDL.to_verify_type[verify].add([klass, meth])
          end
      ...
      RDL.deferred << [klass, :type, type, {wrap: wrap,
                                               typecheck: typecheck, verify: verify, modifies: modifies, pure: pure, assumes: assumes}]
    end
    nil
  end

  def add_mixin_type(mod, meth, typ, wrap: RDL::Config.instance.type_defaults[:wrap], typecheck: RDL::Config.instance.type_defaults[:typecheck], verify: RDL::Config.instance.type_defaults[:verify], modifies: RDL::Config.instance.type_defaults[:modifies], pure: RDL::Config.instance.type_defaults[:pure])
    RDL.mixin_types[mod] = [] unless RDL.mixin_types[mod]
    RDL.mixin_types[mod] << [meth, typ, wrap, typecheck, verify, modifies, pure] # no need to include modular here
    if RDL.included_in[mod]
      RDL.included_in[mod].each { |kl|
        case kl
        when Class
        # TODO: apply to subclasses?
          type(kl, meth, typ, wrap: wrap, typecheck: typecheck, verify: verify, modifies: modifies, pure: pure)
        when Module
          add_mixin_type(kl, meth, typ, wrap: wrap, typecheck: typecheck, verify: verify, modifies: modifies, pure: pure) #TODO: is this right?
        end
      }
    end
  end
  ...
  def assert(*args, &blk)
    raise RuntimeError, "No block given for assertion." if blk.nil?
    klass, meth = RDL::Wrap.process_assert_args(self, *args, &blk)
    if meth
      RDL.info.add(klass, meth, :assert, blk)
      if RDL::Util.method_defined?(klass, meth)
        RDL.info.set(klass, meth, :source_location, RDL::Util.to_class(klass).instance_method(meth).source_location)
        RDL::Verify.verify_assert(klass, meth)
      else
        RDL.to_verify_assert << [klass, meth]
      end
    else
      RDL.deferred << [klass, :assert, blk, {}]
    end
  ...
  def rdl_do_verify(sym, count = nil)
    return unless RDL.to_verify_type[sym]
    if count
      times_per_meth = {}
      med_tot = 0
      siqr_tot = 0
      ...
      RDL.to_verify_type[sym].each { |klass, meth|
        counts = []
        count.times {
          time = Benchmark.measure { RDL::Verify.verify_type(klass, meth) }
          counts << time.real
        }
        sort = counts.sort
        times_per_meth[[klass, meth]] = sort
        puts "#{klass} method #{meth} times over #{count} runs: #{counts}"
        q1 = median(sort[0..sort.length/2])
        med = median(sort)
        q3 = median(sort[sort.length/2..sort.length])
        siqr = (q3-q1)/2
        puts "The median time was: #{med}"
        puts "The SIQR was: #{siqr}"
        med_tot = med_tot + med
        siqr_tot = siqr_tot + siqr
        table << "& #{klass}.#{meth} & RubLOC & RosLOC & #{med.round(2)}& #{siqr.round(2)} & specs\\\\ \n \\cline{2-7} \n"
      }
      puts "The total of the medians was #{med_tot} and the total of the SIQRs was #{siqr_tot}"
      puts "<<<<<<<<<<<<<<<<<TABLE>>>>>>>>>>>>>>>>>>>>"
      puts table
    else
    RDL.to_verify_type[sym].each { |klass, meth|
    ...
    RDL.to_verify_type[sym] = Set.new
    nil
  end
  ...
  def rdl_define_method(name, str_typ, &blk)
