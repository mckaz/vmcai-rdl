...
class TestGeneric < Minitest::Test
  ...
  def test_instantiate
    ...
    assert_raises(RDL::Type::TypeError) {
      ...
      A.new([A.new(["a", "b"]).instantiate!('String'), A.new([])]).instantiate!('TestGeneric::A<String>', check: true)
    }
    assert_raises(RDL::Type::TypeError) {
      # All members must be of same type
      A.new([A.new(["a", "b"]).instantiate!('String'), "A"]).instantiate!('TestGeneric::A<String>', check: true)
    }
    assert_raises(RDL::Type::TypeError) {
      # All members must be instantiated and of same type
      A.new([A.new(["a", "b"]).instantiate!('String', check: true),
             H.new({a: 1, b: 2}).instantiate!('Object', 'Object')]).instantiate!('TestGeneric::A<String>', check: true)
    }
  end
